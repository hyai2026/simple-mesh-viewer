...
- **不透明度 α ∈ (0,1]**：同时作用于该模型的面材质（transparent 动态开关）、
  边材质与点精灵 shader 的 `uAlpha`；默认 1。透明物体仍写深度，遮挡关系保持稳定。
...
已知取舍（v1）：边拾取偏向线段端点附近的候选顶点；被表面遮挡的顶点/边在射线未命中面时
也可能命中 —— 与主流建模软件行为一致。透明排序采用写深度的简化方案，极端半透明叠加
可能有混合次序瑕疵。
